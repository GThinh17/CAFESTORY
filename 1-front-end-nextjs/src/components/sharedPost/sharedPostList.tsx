...
export function SharedPostList() {
  ...
  const fetchSharedPosts = async (cursor: string | null = null) => {
    const res = await axios.get("http://localhost:8080/api/shares/by-user", {
      params: {
        userId,
        size: 9,
    ...
    const posts: Post[] = await Promise.all(
      shares.map(async (share) => {
        const blogRes = await axios.get(
          `http://localhost:8080/api/blogs/${share.blogId}`
        );
  ...
  if (posts.length === 0) return <div>No shared posts</div>;
